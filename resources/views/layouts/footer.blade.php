<footer class="footer-section">
	<div class="container">
		<div class="row">
			<!-- Team Info -->
			<div class="col-md-3">
				<!-- <img src="{{'https://png.pngtree.com/png-clipart/20220823/original/pngtree-green-coconut-sticker-vector-illustration-png-image_8462536.png'}}"
					alt="coconerd" class="footer-logo"> -->
				<img src="{{ asset('images/logos/tp-logo-with-text.png') }}"
					alt="coconerd" class="footer-logo">
				<p class="footer-text">
					Plant Paradise mong muốn mang đến một không gian sống xanh, không chỉ là nơi khơi nguồn cảm hứng và cải thiện tinh thần, mà còn góp phần nâng tầm thẩm mỹ cho không gian sống của bạn
				</p>
				<p>
					Nhóm thực hiện đồ án: <strong>COCONERD</strong><br>
					Lớp: Phát triển hệ thống TMĐT 04
				</p>
			</div>

			<!-- Links -->
			<div class="col-md-3">
				<div class="footer-links">
					<h5>VỀ CHÚNG TÔI</h5>
					<ul>
						<li><a href="#">Giới thiệu</a></li>
						<li><a href="#">Liên hệ</a></li>
						<li><a href="#">Chính sách bảo mật</a></li>
						<li><a href="#">Chính sách bảo hành</a></li>
						<li><a href="#">Phương thức thanh toán</a></li>
						<li><a href="#">Phương thức vận chuyển</a></li>
					</ul>
				</div>
			</div>

			<!-- Contact Info -->
			<div class="col-md-3">
				<div class="footer-contact">
					<h5>LIÊN HỆ</h5>
					<p>Hotline 1: giangth.b21cn308@stu.ptit.edu.vn</p>
					<p>Hotline 2: vanlth.b21cn126@stu.ptit.edu.vn</p>
					<p>Hotline 3: caonv.b21cn176@stu.ptit.edu.vn</p>
					<p>Email: ptitcoconerd@gmail.com</p>
					<p>Địa chỉ: Km 10, Nguyễn Trãi, Thanh Xuân, Hà Nội</p>
					<div class="social-icons">
						<a href="#"><i class="fab fa-facebook-f"></i></a>
						<a href="#"><i class="fab fa-twitter"></i></a>
						<a href="#"><i class="fab fa-instagram"></i></a>
						<a href="#"><i class="fab fa-youtube"></i></a>
					</div>
				</div>
			</div>

			<!-- Map -->
			<div class="col-md-3">
				<h5>ĐỊA ĐIỂM PLANT PARADISE</h5>
				<div class="map-container">
					<iframe src="https://www.google.com/maps/embed?pb=..." width="100%" height="100%" style="border:0;"
						allowfullscreen="" loading="lazy">
					</iframe>
				</div>
			</div>
		</div>

		<!-- Copyright -->
		<div class="copyright">
			<p>Copyright 2025 © COCONERD</p>
		</div>
	</div>
</footer>